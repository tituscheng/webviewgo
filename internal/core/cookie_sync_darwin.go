...
static void syncCookie(void *webViewPtr, const char *name, const char *value,
    ...
    runOnMain(^{
        ...
        if (!wv) return;
        WKHTTPCookieStore *mainStore = wv.configuration.websiteDataStore.httpCookieStore;
        dispatch_semaphore_t sem = dispatch_semaphore_create(0);
        [mainStore setCookie:cookie completionHandler:^{
...
func (w *darwinWebView) SyncCookiesToNative(cookies []types.Cookie) error {
	...
	for _, c := range cookies {
		...
		C.syncCookie(w.webView, name, value, domain, path, expires, cBool(c.Secure), cBool(c.HTTPOnly), cBool(c.HostOnly), C.int(c.SameSite))
